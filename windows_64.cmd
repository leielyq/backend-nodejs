...
if "!PREFERRED_VC_REDIST_ROOT!"=="" (
  if "!PREFERRED_VS_INSTALL_PATH!"=="" (
    ...
    if exist "!VSWHERE!" (
      for /f "usebackq delims=" %%I in (`"!VSWHERE!" -latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath`) do if "!PREFERRED_VS_INSTALL_PATH!"=="" set "PREFERRED_VS_INSTALL_PATH=%%I"
    )
  )
  if "!PREFERRED_VS_INSTALL_PATH!"=="" (
  ...
  if "!PREFERRED_VC_REDIST_ROOT!"=="" (
    for /f "delims=" %%D in ('dir /b /ad /o-n "!VC_REDIST_BASE!\14.*" 2^>nul') do if "!PREFERRED_VC_REDIST_ROOT!"=="" set "PREFERRED_VC_REDIST_ROOT=!VC_REDIST_BASE!\%%D"
  )
  if "!PREFERRED_VC_REDIST_ROOT!"=="" (
    for /f "delims=" %%D in ('dir /b /ad /o-n "!VC_REDIST_BASE!" 2^>nul') do if "!PREFERRED_VC_REDIST_ROOT!"=="" set "PREFERRED_VC_REDIST_ROOT=!VC_REDIST_BASE!\%%D"
  )
)
...
for /d %%D in ("!PREFERRED_VC_REDIST_ROOT!\x64\Microsoft.VC*.CRT") do if "!VC_RUNTIME_SOURCE!"=="" set "VC_RUNTIME_SOURCE=%%~fD"
if "!VC_RUNTIME_SOURCE!"=="" (
  echo Missing VC runtime CRT directory: "!PREFERRED_VC_REDIST_ROOT!\x64\Microsoft.VC*.CRT"
  exit /b 1
)
echo Using Visual C++ runtime DLLs from "!VC_RUNTIME_SOURCE!"
copy /y "!VC_RUNTIME_SOURCE!\concrt140.dll" ".\puerts-node\nodejs\lib\Win64\" || exit /b 1
